...
const APIURL = "https://api.studio.thegraph.com/query/69527/temp_graph/v0.0.13";
...
export const SafeBuyProvider = ({ children }) => {
  ...
  const registerCompany = async (comAdd, name, cin) => {
    const contract = await connectingWithSmartContract();
    if (currentAccount) {
      const transaction = await contract.registerCompany(comAdd, name, cin);
      const receipt = await transaction.wait();
      console.log(receipt);
    }
  };
  ...
  const acceptCompany = async (comAdd) => {
    const contract = await connectingWithSmartContract();
    if (currentAccount) {
		const transaction = await contract.acceptCompany(comAdd);
		const receipt = await transaction.wait();
      	console.log(receipt);
    }
  };
  ...
  const rejectCompany = async (comAdd) => {
    const contract = await connectingWithSmartContract();
    if (currentAccount) {
		const transaction = await contract.rejectCompany(comAdd);
		const receipt = await transaction.wait();
      	console.log(receipt);
    }
  };
  ...
  const addBulkProducts = async (
    ...
    const contract = await connectingWithSmartContract();
	if(contract && currentAccount){
		const transaction = await contract.addBulkProducts(
			productId,
			pubKeys,
			privateKeys,
			manDate,
  ...
  const buyProduct = async (privateKey, tokenURI) => {
    const contract = await connectingWithSmartContract();
    const transaction = await contract.buyProduct(privateKey, tokenURI);
	const receipt = await transaction.wait();
	console.log(receipt);
  };
